...
The web demo is a visual reading of the bundled sample. It creates no localStorage, sessionStorage, IndexedDB, or files, so it has no demo or real-data namespace to read or write. Its banner labels the sample mode, offers Reset demo, and offers Start for real. `/demo` is the direct verifier URL.
